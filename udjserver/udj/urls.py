...
urlpatterns += patterns('udj.views.event',
  ...
  (r'^events/(?P<event_id>\d+)/users/(?P<user_id>\d+)$', 'leaveEvent'),
...
urlpatterns += patterns('udj.views.activeplaylist',
  ...
  (r'^events/(?P<event_id>\d+)/active_playlist/(?P<playlist_id>\d+)/downvote$', 
    'voteSongDown'),
  (r'^events/(?P<event_id>\d+)/active_playlist/(?P<playlist_id>\d+)$', 
    'removeSongFromActivePlaylist'),
  (r'^events/(?P<event_id>\d+)/active_playlist/(?P<user_id>\d+)/add_requests$', 
    'getAddRequests'),
)
