...
class BasicPlayerAdministrationTests(DoesServerOpsTestCase):
  ...
  def testSetVolume(self):
    ...
    response = self.doPost('/udj/0_7/players/1/volume', {'volume' : 2})
    self.assertEqual(response.status_code, 200)
    player = Player.objects.get(pk=1)
  ...
  def testBadVolumeSet(self):
    player = Player.objects.get(pk=1)
    self.assertEqual(player.volume, 5)
    response = self.doPost('/udj/0_7/players/1/volume', {'volume' : 11})
  ...
  def testSetPassword(self):
    ...
    response = self.doPost('/udj/0_7/players/1/password', {'password': newPassword})
    self.assertEqual(response.status_code, 200)
  ...
  def testSetLocation(self):
    ...
    response = self.doPost('/udj/0_7/players/1/location', newLocation)
    self.assertEqual(response.status_code, 200)
  ...
  def testSetLocationWithNoPreviousLocation(self):
    ...
    response = self.doPost('/udj/0_7/players/4/location', newLocation)
  ...
  def testKickNonParticipatingUser(self):
    response = self.doPut('/udj/0_7/players/1/kicked_users/1')
    self.assertEqual(404, response.status_code)
    self.assertEqual('user', response[MISSING_RESOURCE_HEADER])

  def testKickNonExistentUser(self):
    response = self.doPut('/udj/0_7/players/1/kicked_users/100000')
    self.assertEqual(404, response.status_code)
    self.assertEqual('user', response[MISSING_RESOURCE_HEADER])

  def testBanParticipatingUser(self):
    response = self.doPut('/udj/0_7/players/1/banned_users/3')
    self.assertEqual(201, response.status_code)
  ...
  def testBanNonParticipatingUser(self):
    response = self.doPut('/udj/0_7/players/1/banned_users/1')
    self.assertEqual(201, response.status_code)
    bannedUser = Participant.objects.get(user__id=1, player__id=1)
    self.assertEqual(True, bannedUser.ban_flag)

  def testBanNonExistentUser(self):
    response = self.doPut('/udj/0_7/players/1/banned_users/10000000')
    self.assertEqual(404, response.status_code)
    self.assertEqual('user', response[MISSING_RESOURCE_HEADER])

  def testUnbanUser(self):
    response = self.doDelete('/udj/0_7/players/1/banned_users/8')
    self.assertEqual(200, response.status_code)
  ...
  def testUnbanNonParticipatingUser(self):
    response = self.doDelete('/udj/0_7/players/1/banned_users/1')
    self.assertEqual(404, response.status_code)
    self.assertEqual('user', response[MISSING_RESOURCE_HEADER])

  def testUnbanNonExistentUser(self):
    response = self.doDelete('/udj/0_7/players/1/banned_users/1000000')
    self.assertEqual(404, response.status_code)
    self.assertEqual('user', response[MISSING_RESOURCE_HEADER])

  def testGetBannedUsers(self):
    response = self.doGet('/udj/0_7/players/1/banned_users')
    self.assertEqual(200, response.status_code)
