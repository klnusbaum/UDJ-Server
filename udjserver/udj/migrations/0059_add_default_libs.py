...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
      ...
      for player in orm.Player.objects.all():
        new_library = orm.Library(name=player.name+" library", description="default library for player", pub_key="")
        new_library.save()
        new_default = orm.DefaultLibrary(library=new_library, player=player)
        new_default.save()
        new_associate = orm.AssociatedLibrary(library=new_library, player=player)
        new_associate.save()
        new_owned = orm.OwnedLibrary(library=new_library, owner=player.owning_user)
        new_owned.save()
    ...
    def backwards(self, orm):
      for player in orm.Player.objects.all():
        default_lib = orm.DefaultLibrary.objects.get(player=player)
        default_lib.library.delete()
