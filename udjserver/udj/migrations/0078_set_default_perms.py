...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
      for player in orm.Player.objects.all():
        owner_group = orm.PlayerPermissionGroup(player=player, name='owner')
        owner_group.save()
        owner_group_member = orm.PlayerPermissionGroupMember(permission_group=owner_group,
                                                             user = player.owning_user)
        def create_perm(perm_name):
          new_perm = orm.PlayerPermission(player=player, permission=perm_name, group=owner_group)
          new_perm.save()
        map(create_perm, DEFAULT_PLAYER_PERMISSIONS)

        if player.allow_user_songset:
          orm.PlayerPermission.objects.filter(player=player, permission=u'CSS').delete()



    def backwards(self, orm):
      orm.PlayerPermissionGroup.objects.all().delete()
