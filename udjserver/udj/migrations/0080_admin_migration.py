...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
      for player in orm.Player.objects.all():
        admins = orm.PlayerAdmin.objects.filter(player=player)
        if admins.exists():
          admin_group = orm.PlayerPermissionGroup(player=player, name=u'admin')
          map(lambda x: admin_group.add_member(x), admins)
          for permission in DEFAULT_PLAYER_PERMISSIONS:
            newPermission = orm.PlayerPermission(permission_group=admin_group, permission=permission)

    def backwards(self, orm):
      PlayerPermissionGroup.objects.filter(name=u'admin').delete()
